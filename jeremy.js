...
var searchTidal = function(query, limit) {
  if (!limit) limit = 20;
  return fetch(TIDAL_BACKEND + "/search?q=" + encodeURIComponent(query) + "&limit=" + limit)
    .then(r => r.json())
    .then(data => {
      return {
...
var getTidalStream = function(trackId) {
  return fetch(TIDAL_BACKEND + "/stream/" + trackId)
    .then(r => r.json())
...
return {
  ...
  labels: ["QOBUZ", "TIDAL", "HI-RES", "SMART", "PRELOAD", "ALBUMS"],
  ...
  searchArtists: function(query, limit) {
    if (!limit) limit = 10;
    return fetch(TIDAL_BACKEND + "/search/artists?q=" + encodeURIComponent(query) + "&limit=" + limit)
      .then(r => r.json())
  ...
  searchPlaylists: function(query, limit) {
    if (!limit) limit = 10;
    return fetch(TIDAL_BACKEND + "/search/playlists?q=" + encodeURIComponent(query) + "&limit=" + limit)
      .then(r => r.json())
