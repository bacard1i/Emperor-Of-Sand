...
function getFullTitle(item) {
  ...
  if (ver && base.toLowerCase().indexOf(ver.toLowerCase()) === -1) return base + " " + ver;
  return base;
}
...
// ==================== IMPROVED MERGE (Now shows Tidal results) ====================
function mergeSmart(qobuzTracks, tidalTracks, limit) {
  ...
  // Add Tidal tracks only if they have a different ISRC
  tidalTracks.forEach(function(t) {
    var key = t.isrc || normalizeQ(t.title + "|" + t.artist);
    
    if (t.isrc && seenISRC.has(t.isrc)) return;     // Same ISRC → skip
    if (seenKeys.has(key)) return;                  // Exact same song → skip
...
return {
  ...
  author: "bacardii",
  version: "3.1",
  description: "Qobuz Hi-Res Primary + Tidal Fallback • Shows unique Tidal results (Molecule Mouth etc)",
  labels: ["QOBUZ", "TIDAL", "HI-RES", "SMART", "ALBUMS"],
